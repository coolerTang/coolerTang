...
class Tree {
    ...
    function get_treeview_menu($myid,$effected_id='example', $str="<span class='file'>\$name</span>", $str2="<span class='folder'>\$name</span>", $showlevel = 0,  $ul_class="" ,$li_class="" , $style='filetree ', $currentlevel = 1, $recursion=FALSE, $dropdown='hasChild',$class='active') {
	...
	$placeholder = '<ul><li><span class="placeholder"></span></li></ul>';
    	...
    	foreach ($child as $id => $a) {
    		...
    		if ($this->get_child($a['id'])) {
    			...
    			eval("\$nstr = \"$str\";");
                if(strpos($nstr,CONTROLLER_NAME)){
                    $par='/href=(["\'].*?["\'])/is';
                    $this->str.=preg_replace($par,'href=$1 class="'.$class.'" ',$nstr);
                }else{
                    $this->str.=$nstr;
                }
    		}
