...
function sp_sql_posts_paged($type,$tag,$pagesize=20,$pagetpl='{first}{prev}{liststart}{list}{listend}{next}{last}'){
	$arr['path']=array('like','%'.$type.'%');
	$res=M('terms')->where($arr)->field('term_id')->select();
	if($res){
		$termId=array_map(function($value){
		return $value['term_id'];
		},$res);
		$termIdStr=implode($termId,',');
		$where['post_parent'] = array('in',$termIdStr);
	}
	$tag=sp_param_lable($tag);
	$field = !empty($tag['field']) ? $tag['field'] : '*';
	$limit = !empty($tag['limit']) ? $tag['limit'] : '';
	...
	$rs= M("posts");
	$join = "".C('DB_PREFIX').'terms as b on a.post_parent =b.term_id';
